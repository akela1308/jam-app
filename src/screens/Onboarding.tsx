...
export function Onboarding({ onDone }: OnboardingProps) {
  ...
  const ACCENTS = ['#DF1760', '#7C3AED', '#F59E0B'];
  const BGS = [
    'linear-gradient(160deg, #fff0f6 0%, #ffe4ef 100%)',
    'linear-gradient(160deg, #f3f0ff 0%, #e8e4ff 100%)',
  ...
  return (
    ...
    >
      ...
      <div className="onboarding__lang-pick">
        <button
          className={`onboarding__lang-btn${lang === 'hr' ? ' onboarding__lang-btn--active' : ''}`}
          onClick={() => setLang('hr')}
        >
          🇭🇷 HR
        </button>
        <button
          className={`onboarding__lang-btn${lang === 'ru' ? ' onboarding__lang-btn--active' : ''}`}
          onClick={() => setLang('ru')}
        >
          🇷🇺 RU
        </button>
        ...
        {slides.map((s, i) => (
          ...
          >
            <div className="onboarding__illustration">
              {i === 0 && <ReviewsIllustration />}
              {i === 1 && <CouponsIllustration />}
              {i === 2 && <CommunityIllustration lang={lang} />}
            </div>
            <div className="onboarding__text">
              <div className="onboarding__emoji">{s.emoji}</div>
              <h2 className="onboarding__title" style={{ color: ACCENTS[i] }}>
                {s.title}
              </h2>
...
function CouponsIllustration() {
  return (
    <svg viewBox="0 0 280 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="onboarding__svg">
      ...
      <text x="44" y="100" fontSize="10" fill="rgba(255,255,255,0.7)">do 31 pro</text>
...
function CommunityIllustration({ lang }: { lang: string }) {
  const bubble1 = lang === 'ru' ? 'Купон!' : 'Kupon!';
  const bubble2 = lang === 'ru' ? 'Классно! 💛' : 'Super! 💛';
  const bubble3 = lang === 'ru' ? '5 ★ отзыв' : '5 ★ recenzija';
  const bubble4 = lang === 'ru' ? 'Топчик 🌿' : 'Odlično 🌿';
  return (
    <svg viewBox="0 0 280 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="onboarding__svg">
      ...
      <text x="56" y="41" textAnchor="middle" fontSize="11" fill="#374151">{bubble1}</text>
      <rect x="180" y="14" width="86" height="32" rx="16" fill="white" filter="url(#sh2)" />
      <text x="223" y="35" textAnchor="middle" fontSize="11" fill="#374151">{bubble2}</text>
      <rect x="24" y="130" width="90" height="32" rx="16" fill="white" filter="url(#sh2)" />
      <text x="69" y="151" textAnchor="middle" fontSize="10" fill="#374151">{bubble3}</text>
      <rect x="168" y="138" width="90" height="32" rx="16" fill="white" filter="url(#sh2)" />
      <text x="213" y="159" textAnchor="middle" fontSize="10" fill="#374151">{bubble4}</text>
